...
import com.example.simbirsoftpracticeapp.exceptions.NotImplementedException;
...
public class RxCombiningTraining {
    ...
    public Observable<Integer> additionalFirstItem(int firstItem, Observable<Integer> intObservable) {
        return Observable.fromArray(firstItem).mergeWith(intObservable);
    }
    ...
    List<String> searchItems(String searchString, int categoryId) {
        ...
        for (String word : words) {
            if (word.contains(Integer.toString(categoryId))) {
                result.add(word);
            }
        }
        return result;
